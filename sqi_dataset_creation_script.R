...
constraints <- read_csv("strm_constraints_trim.csv") %>% # Modeled CSCI values generated by Marcus Beck may be accessed at: https://knb.ecoinformatics.org/view/urn%3Auuid%3A67368ec2-0e33-4088-a058-921ef35a70be#urn%3Auuid%3Ac510fd23-032e-40f3-aaa0-e1a42241ddef.
  rename(comid = COMID) %>%
  rename(strcls = Ref10) %>%
  rename(lower = qt10) %>%
  rename(meds = qt50) %>%
...
# Load lu_stations so we can get the MasterIDs until Robert pulls them with the dataset.
lu_stations <- read_csv("/Users/heilil/Desktop/smc_datasets/lu_stations_prob.csv") %>%
...
sqidat <- sqi_query %>%
  ...
  left_join(lu_stations, by = c("stationcode" = "stationid")) %>%
...
sqidat3 <- sqidat2 %>%
  ...
  # Convert longitude and latitude to sf geometries
